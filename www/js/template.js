...
tabbarMenu = function(){
	...
	html+='<ons-tabbar position="bottom" animation="none" modifier="is_rtl" >';
	    ...
	    html+='</ons-tab>';	    
	    html+='<ons-tab page="order_list.html" label="'+ t("Order") +'" icon="ion-pizza" active-icon="ion-pizza" ></ons-tab>';	    
	    html+='<ons-tab page="profile.html" label="'+ t("Account") +'" icon="ion-android-contact" active-icon="ion-android-contact" >';
	    html+='</ons-tab>';	    
	    html+='<ons-tab page="cart_temp.html" label="'+ t("Cart") +'" icon="ion-bag" active-icon="ion-bag" badge="" >';
	    html+='</ons-tab>';
    html+='</ons-tabbar>';	
...
fillRestoPageInfo = function(data){
	...
	if(!empty(data.delivery_fee)){
	   html +='<p class="bold">'+ data.delivery_fee +'</p>';
	}
...
restaurantCategory = function(data, element){
	...
	$.each( data  , function( key, val ) {
		 ...
		 html+='<ons-list-item expandable modifier="list_item_category" class="'+class_expanded+'" >';
		   ...
		   if(val.item.length>0){
			     ...
			     if(!empty(item_val.item_description)){
			         ...
			         if(code_version>="1.4"){
			         	if (item_val.prices2.length>0 && !website_hide_foodprice){
			         		html+='<br/>';
				         ...
				         if (item_val.prices.length>0 && !website_hide_foodprice){
	...
	$.each( data  , function( key, val ) {
		...
		html+='<ons-list-item modifier="longdivider" tappable onclick="itemDetails('+ "'"+ val.item_id+"'," + "'" + val.cat_id + "'"  +')"  >';
		  ...
		  html+='<div class="center">';
		    ...
		    if (val.prices.length>0 && !website_hide_foodprice){
		    	html+='<span class="list-item__subtitle">';
...
var displayItemDetails = function(data, cart_data) {
	...
	enabled_dish=''; social_share_food='';
	if(app_settings = getAppSettings()){
		enabled_dish = app_settings.enabled_dish;
		social_share_food = app_settings.social_share_food;
	}
	...
	html+='<div class="wrap">';	
	 ...
	 if(social_share_food==1){
		  html+='<ons-row>';
		    html+='<ons-col vertical-align="center" width="50%">';
		    html+= '<b>'+data.item_name+'</b>';
		    html+='</ons-col>';	    	    
		    if ((typeof  data.share_options !== "undefined") && ( data.share_options !== null)) {	    	
			    params = clickFormat( data.share_options.message +"|"+ data.share_options.subject + "|" + data.share_options.url );		    
			    html+='<ons-col vertical-align="center" width="50%" class="to_right">';
			    html+='<ons-button modifier="quiet icon_to_orange" onclick="socialSharing('+ params +')" >';
			       html+='<ons-icon icon="ion-android-share-alt" size="22px" ></ons-icon>';
			    html+='</ons-button>';
			    html+='</ons-col>';		    
		    }	    
		  html+='</ons-row>';
		    	...
		    	$.each( data.prices, function( price_key, price_val ) {
		    		...
		    		html+='<ons-list-item tappable>';
		    		  html+='<label class="left">';
				        ...
				        html+='<ons-radio name="price" modifier="inv_price" input-id="price-'+price_key+'" value="'+ value_price +'"  '+selected+' onclick="inv_sizePrice('+ q(value_price) +');" ></ons-radio>';
				      html+='</label>';
...
var displayCartDetails = function(datas){
	...
	if(!empty(data.item)) {		
		$.each( data.item, function( item_key, item_val ) {
			...
			if(!$cart_with_qty){
				...
				$item_name =  item_val.item_name;
				if ( item_val.discount>0 ){
				   $item_name+='<p class="item_details">' +  item_val.size_words + " "  +'<span class="tag_discount">'+prettyPrice(item_val.normal_price)+'</span> ' + prettyPrice(item_val.discounted_price) + '</p>';
				} else {
				  $item_name+='<p class="item_details">' + item_val.size_words + " " + prettyPrice(item_val.normal_price) + '</p>';
				}
				
				html+='<ons-list-item  modifier="nodivider" >';
				html+='<ons-row>';
				  html+='<ons-col vertical-align="center" width="45%" onclick="itemDetails('+ "'" + item_val.item_id +  "'," + "'" + item_val.category_id + "'," + "'" + item_key  + "'"   +')"  >'+ $item_name +' </ons-col>';				  
				  html+='<ons-col vertical-align="center" width="25%">';
				  
				 
				   html+='<ons-row class="button_cart" >';
			          html+='<ons-col vertical-align="center" width="33.3%" >';
			          html+='<ons-button modifier="quiet"onclick="directQty( '+ item_val.item_id +', $(this), 0 ,'+q(item_val.category_id) + "," + q(item_key) +' )"  ><ons-icon icon="md-minus" size="16px" ></ons-icon></ons-button>';
			          html+='</ons-col>';
			          html+='<ons-col vertical-align="center" width="33.3%" >';
			          html+='<ons-input name="qtys" id="qtys" modifier="transparent" value="'+ item_val.qty +'" readonly="true"></ons-input>';
			          html+='</ons-col>';
			          html+='<ons-col vertical-align="center" width="33.3%" >';
			          html+='<ons-button modifier="quiet" onclick="directQty( '+ item_val.item_id +', $(this), 1 ,'+q(item_val.category_id)  + "," + q(item_key) +' )"  ><ons-icon icon="md-plus" size="16px" ></ons-icon></ons-button>';
			          html+='</ons-col>';
			        html+='</ons-row>';
				  
				  html+='</ons-col>';
				  html+='<ons-col vertical-align="center" class="to_right" width="30%">'+  prettyPrice( parseFloat(price_used)*parseFloat(item_val.qty) )  +'</ons-col>';
				html+='</ons-row>';
				html+='</ons-list-item>';
			}
			...
			if (!empty(item_val.cooking_ref)){
				html+='<ons-list-item modifier="nodivider list_details" >';
			...
			if (!empty(item_val.ingredients)){
				html+='<ons-list-item modifier="nodivider list_details" >';
				html+= '<div class="left">'+ t("Ingredients") + ": " ;
				ingredients_list='';   
	...
	if(!is_apply_tax){
	   html+= twoColumn( t('Sub Total') ,  prettyPrice(data.total.subtotal) ,'longdivider' );
	}
	...
	if(!is_apply_tax){
		if (!empty(data.total.taxable_total)){
			if(data.total.taxable_total>0.0001){
			   ...
			   html+= twoColumn( t('Tax') + " " + number_format($tax,2,'.','') + "%" ,  prettyPrice(data.total.taxable_total) );
			}
		}
	}
	...
	if (!empty(data.total.total)){
		if(data.total.total>0){
		   html+= twoColumn( t('Total') ,  prettyPrice(data.total.total) ,'longdivider' );
		   html+='<ons-list-item modifier="divider" ></ons-list-item>';		   
	...
	html+='<ons-list-item tappable modifier="contactless nodivider" vertical-align="top"  >';
	...
	html+='<ons-list-item modifier="chevron nodivider" tappable onclick="showTransactionList()" >';	
	...
	html+='<ons-list-item tappable modifier="chevron nodivider" onclick="showDeliveryDateList()" >';	
	...
	html+='<ons-list-item tappable modifier="chevron nodivider" onclick="showDeliveryTime()" >';	
	...
	if ( datas.transaction_type == "delivery" ){
		...
		html+='<ons-list-item tappable modifier="chevron nodivider" onclick="initAddress()" >';	
		  html+='<div class="left">' + t("Delivery Address")  +'</div>';
		   html+='<div class="center">';
		   html+='<span class="list-item__subtitle delivery_address_label concat_text">'+ selected_delivery_address +'</span>';		   		   
		   html+='</div>';
		html+='</ons-list-item>';
	}
...
var twoColumn = function(label, value, modifier){
	if ((typeof  modifier !== "undefined") && ( modifier !== null)) {
		//
	} else {
		modifier = 'nodivider';
	}
	var html='<ons-list-item modifier="'+ modifier +'">';
...
tipColumn = function(data){
	...
	html+='<ons-list-item modifier="nodivider">';
	 ...
	 html+='<h3>'+ t("Support delivery executive") +'</h3>';
	 html+='<p>'+ t("Thank your delivery executive for helping you stay safe indoors. Support them through these tought times with tip.") +'</p>';
	html+='</div>';	
	html+='</ons-list-item>';
	
	html+='<div class="tip_carousel">';
	
	html+='<ons-carousel fullscreen swipeable auto-scroll overscrollable id="" item-width="23%"  >';
	
	$.each( data, function( key, val ) {
	  html+='<ons-carousel-item onclick="applyTips('+ q(key) +')" >';
	...
	html+='<ons-list>';
	  $.each( data, function( key, val ) {
	  	 if (transaction_type=="delivery_time"){
	  	 	...
	  	 	html+='<ons-list-item tappable modifier="longdivider" onclick="setFieldValue(' + "'" + transaction_type + "'," + "'" + key + "','" + addslashes(val) + "'"  + ' )" ><div class="center">'+ val +'</div></ons-list-item>';
	  	 } else {
...
accountMenu = function(login){
	    ...
	    if(app_settings.app_rating.enabled==1 && $only_user_login==true){
		 html+='<ons-list modifier="list_menu">';
	      html+='<ons-list-item  tappable onclick="rateApp()" >';
	        ...
	        html+='<div class="center">' + t('Rate Us') +  '</div>';
	      html+='</ons-list-item>';
		}
	}
	...
	if (login){
		 ...
		 html+='<ons-list modifier="list_menu">';
	      ...
	      html+='<ons-list-item modifier="" tappable  onclick="logout();" >';
...
settingsMenu = function(login){
	...
	if(login){
      ...
      html+='<ons-list-item >';
        ...
        html+='<div class="center"><span class="list-item__subtitle">'+ t("Receive notification about your order") +'</span></div>';
        html+='<div class="right">';
      ...
      html+='<ons-list-item >';
        html+='<div class="left"><ons-icon icon="md-notifications-active" size="22px"></ons-icon></div>';
        html+='<div class="center"><span class="list-item__subtitle">'+ t("Receive alert about promo,news and other exciting offer") +'</span></div>';
        html+='<div class="right">';
...
setBookingList = function(data, element){
	...
	$.each( data  , function( key, val ) {
	    ...
	    html+='<ons-list-item modifier="full_list list_style1" tappable  onclick="actionSheetBooking('+ params +')" >';
		html+='<div class="inner">';
		   html+='<ons-row>';
		     html+='<ons-col width="60px" vertical-align="center" >'
...
merchantAbout = function(data, element){
	...
	if ((typeof  data.background_url !== "undefined") && ( data.background_url !== null)) {
	   ...
	   html+='<ons-list>';
	    ...
	    if(data.payment.length>0){
	    	$.each( data.payment  , function( key, val ) {
			    ...
			    if ((typeof  val.provider_list !== "undefined") && ( val.provider_list !== null)) {
			    	if(val.provider_list.length>0){
			    		html+='<ons-list-item modifier="nodivider">';
			    		html+='<ons-row>';
			    		$.each( val.provider_list  , function( $key, $val ) {			    			
			    			html+='<ons-col width="65px" vertical-align="center" >';
			    			html+='<img class="provider_img" src="'+ $val.payment_logo +'">';
			    			html+='</ons-col>';
			    			
			    			html+='<ons-col vertical-align="center" >';
			    			html+=$val.payment_name;
			    			html+='</ons-col>';
	...
	$.each( data  , function( key, val ) {
		...
		html+='<ons-list-item tappable modifier="longdivider full_list" >';
		  ...
		  html+='<label for="radio-'+key+'" class="center">';
		    ...
		    html +='</div>';
		    html+='<p class="list-item__subtitle small">'+val.payment_name+'</p>';		        
		  html+='</label>';
		html+='</ons-list-item>';
		 ...
		 col+='<ons-col width="50%" vertical-align="top" onclick="loadMerchant('+ val.merchant_id+')" >';
			col+='<div class="banner">';
              ...
              if(!empty(val.sponsored)){
			   ...
			   if(!empty(val.minimum_order_raw)){
				    if (val.minimum_order_raw>0){
					    col +='<div class="min_tag">';
					     col += val.minimum_order+'<br><small>MIN</small>';
					    col +='</div>';
				    }  
			    }
			   ...
			   if( !empty(val.offers)){
			       ...
			       $.each( val.offers  , function( key_offer, val_offer ) {
			       	  if(xoffer<=0){
			       	     col +='<div class="pink_tag ">'+ val_offer.raw +'</div>';			       	  			       	 
			       	  }
			       	  xoffer++;
			       });			       
			    }
...
fillOrderTabs = function(element, tab, selected_index){	
	...
	$("#order_tabs").find("ons-carousel-item").remove();
...
fillHomeBanner = function(data, div){
	...
	$.each( data  , function( key, val ) {
      ...
      html+='<div class="banner">';
        ...
        if(!empty(val.banner)){
        	html+='<div class="header_bg" style="background-image: url('+ "'" + val.banner + "'" +')"  >';
        } else {
...
setFoodPromoCarousel = function(element, data){
	...
	$.each( data, function( key, val ) {
		...
		html+='<ons-carousel-item onclick="loadMerchantWithFood2('+ params_data +')" >';	
	    html+='<ons-ripple color="#EF6625" background="#EF6625"></ons-ripple>';	
	    
	     html +='<div class="ribbon"><span>'+ t("PROMO") +'</span></div>';
		  ...
		  $.each( val.prices2, function( price_key, price ) {
		  	html+='<p class="bold concat_text"><span class="tag_discount">'+price.original_price+'</span>'+ price.discounted_price_pretty +'</p>';	
		  });		  
	  html+='</ons-carousel-item >';
	});
	html+='</ons-carousel>';
...
fillCategories = function( data , div){
	html=''; $x=1; $xx=1; col='';
	
	data_count = parseInt(data.length) + 0 ;   
	...
	if(data_count>0){
		...
		$.each( data  , function( key, val ) {
			  ...
			  col+=$do_actions;
		       ...
		       col+='</div>';
		       col+='<p>'+ val.title +'</p>';
		        col+='<ons-ripple color="#b9d4f0" background="#ffdb79"></ons-ripple>';
		      col+='</ons-col>';
...
setItemListWithQty = function(data, element){
	...
	$.each( data  , function( key, val ) {
		...
		html+='<ons-list-item modifier="nodivider food_list_price" >';
		html+='<ons-row>';
		
		  if(!empty(val.photo)){
		  html+='<ons-col vertical-align="top" width="85px" onclick="itemDetails('+ "'"+ val.item_id+"'," + "'" + val.cat_id + "'"  +')"  >';		  
		  html +='<div class="is-loading small-loader">'; 
		       html +='<div class="spinner"></div>';		
		       html+='<img class="list-item__thumbnail" src="'+ val.photo +'">';
		    html+='</div>';		 
		  ...
		  html+='<ons-col vertical-align="top" >';		  
			  ...
			  html+='<ons-row  >'; 
			  html+='<p class="list-item__title title" >'+ val.item_name +'</p>';
			  if(!empty(val.item_description)){
			  html+='<p class="list-item__title description" >'+ val.item_description +'</p>';
			  }
			  html+='</ons-row>';
		  
		    html+='<ons-row>';		        		        
		        html+='<ons-col vertical-align="bottom" >';
		        
		        html+='<p class="list-item__title price" >';
		        if (val.prices2.length>0 && !website_hide_foodprice){
	    			$.each( val.prices2  , function( pricekey, priceval ) {
	    				if(priceval.discount>0.001){
						  	html+= '<span class="tag_discount">'+ priceval.original_price+'</span><br/>' + priceval.discounted_price_pretty + '<br/>';
						  } else {
						  	html+= priceval.original_price + '<br/>';
						  }			    		  
	    			});
		        ...
		        if(!website_hide_foodprice){
			        html+='<ons-col vertical-align="bottom" width="90px" >';		        
			        if(!val.customizable){		        	
			        	$qty = 0;		        	
			        	if ((typeof  val.added_qty !== "undefined") && ( val.added_qty !== null)) {
			        		$qty = val.added_qty
			        	}		        	
			        	$class_name = "button_cart_"+val.cat_id+"_"+val.item_id;		        			        
				        html+='<ons-row class="button_cart '+ $class_name +'" >';
				          html+='<ons-col vertical-align="center" width="33.3%" >';
				          html+='<ons-button modifier="quiet" onclick="directQty( '+ val.item_id +', $(this), 0 ,'+q(val.cat_id)+' )"  ><ons-icon icon="md-minus" size="16px" ></ons-icon></ons-button>';
				          html+='</ons-col>';
				          html+='<ons-col vertical-align="center" width="33.3%" >';
				          html+='<ons-input name="qtys" id="qtys" modifier="transparent" value="'+$qty+'" readonly="true"></ons-input>';
				          html+='</ons-col>';
				          html+='<ons-col vertical-align="center" width="33.3%" >';
				          html+='<ons-button modifier="quiet" onclick="directQty( '+ val.item_id +', $(this), 1 ,'+q(val.cat_id)+' )" ><ons-icon icon="md-plus" size="16px" ></ons-icon></ons-button>';
				          html+='</ons-col>';
				        html+='</ons-row>';
			        } else {
			        	html+='<ons-button modifier="button_add" onclick="itemDetails('+ "'"+ val.item_id+"'," + "'" + val.cat_id + "'"  +')"  >'+ t("Add") +'</ons-icon></ons-button>';
			        }
			        html+='</ons-col>';		
		        }
...
setItemListColumnWithQty = function(data, element){
	...
	$.each( data  , function( key, val ) {
         ...
         col+='<ons-col width="50%" vertical-align="top" >';
			col+='<div class="banner" onclick="itemDetails('+ "'"+ val.item_id+"'," + "'" + val.cat_id + "'"  +')"   >';
			...
			if(code_version>="1.4"){
				if (val.prices2.length>0 && !website_hide_foodprice){
					...
					$.each( val.prices2  , function( pricekey, priceval ) {		    		
						  if(priceval.discount>0.001){
						  	$prices_list+= '<span class="tag_discount" style="margin-left:0px;">'+ priceval.original_price+'</span><br/>' + priceval.discounted_price_pretty + '<br/>';
						  } else {
						  	$prices_list+= ''+priceval.original_price + '<br/>';
						  }			    		  
			    	});
			    	$prices_list+='</p>';
				}
		...
		if(!val.customizable){		        	
        	...
        	col+='<ons-row style="margin-top:15px;">';
        	col+='<ons-col vertical-align="top" width="50%" >';
        	col+= $prices_list;
        	col+='</ons-col>';
        	
	        col+='<ons-col vertical-align="center" width="50%" >';
        	
	        col+='<ons-row class="button_cart '+ $class_name +'"  >';
	          col+='<ons-col vertical-align="center" width="33.3%" >';
	          col+='<ons-button modifier="quiet" onclick="directQty( '+ val.item_id +', $(this), 0 ,'+q(val.cat_id)+' )"  ><ons-icon icon="md-minus" size="16px" ></ons-icon></ons-button>';
	          col+='</ons-col>';
	          col+='<ons-col vertical-align="center" width="33.3%" >';
	          col+='<ons-input name="qtys" id="qtys" modifier="transparent" value="'+$qty+'" readonly="true"></ons-input>';
	          col+='</ons-col>';
	          col+='<ons-col vertical-align="center" width="33.3%" >';
	          col+='<ons-button modifier="quiet" onclick="directQty( '+ val.item_id +', $(this), 1 ,'+q(val.cat_id)+' )" ><ons-icon icon="md-plus" size="16px" ></ons-icon></ons-button>';
	          col+='</ons-col>';
	        col+='</ons-row>';
        	...
        	col+='<ons-col vertical-align="center" width="50%" class="to_right" style="padding:0;" >';
        	col+='<ons-button modifier="button_add" style="width:auto;" onclick="itemDetails('+ "'"+ val.item_id+"'," + "'" + val.cat_id + "'"  +')"  >'+ t("Add") +'</ons-icon></ons-button>';        	
        	col+='</ons-col>';
	        col+='</ons-row>';
        }		
		}
...
restaurantCategoryWithQty = function(data, element , $cart_with_qty){
	if (data.length<=0){
		return;
	}	
	...
	$.each( data  , function( key, val ) {
		 ...
		 html+='<ons-list-item expandable modifier="list_item_category" class="'+class_expanded+'" >';
		   ...
		   if(val.item.length>0){
			    ...
			    if($cart_with_qty){			    	
				 	if(!item_val.customizable && !website_hide_foodprice){							 		
				 		html+='<span class="list-item__subtitle">';
				 		
				 		$qty = 0;		        	
						if ((typeof  item_val.added_qty !== "undefined") && ( item_val.added_qty !== null)) {
							$qty = item_val.added_qty
						}
						
						$class_name = "button_cart_"+item_val.cat_id+"_"+item_val.item_id;
											
						html+='<ons-row class="button_cart '+ $class_name +'" style="width:110px;" >';
						  html+='<ons-col vertical-align="center" width="33.3%" >';
						  html+='<ons-button modifier="quiet" onclick="directQty( '+ item_val.item_id +', $(this), 0 ,'+q(item_val.cat_id)+' )"  ><ons-icon icon="md-minus" size="16px" ></ons-icon></ons-button>';
						  html+='</ons-col>';
						  html+='<ons-col vertical-align="center" width="33.3%" >';
						  html+='<ons-input name="qtys" id="qtys" modifier="transparent" value="'+$qty+'" readonly="true"></ons-input>';
						  html+='</ons-col>';
						  html+='<ons-col vertical-align="center" width="33.3%" >';
						  html+='<ons-button modifier="quiet" onclick="directQty( '+ item_val.item_id +', $(this), 1 ,'+q(item_val.cat_id)+' )" ><ons-icon icon="md-plus" size="16px" ></ons-icon></ons-button>';
						  html+='</ons-col>';
						html+='</ons-row>';
...
restaurantListWithBannerAll = function(data){	
	html='';
	$.each(data, function(key, val){		
